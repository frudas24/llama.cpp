...
void read_column_f32(const ggml_tensor * W, int64_t j, std::vector<float> & out) {
    ...
    if (!traits->is_quantized) {
        ...
        if (W->type == GGML_TYPE_F16) {
            const auto * v = (const ggml_fp16_t *) base;
            for (int64_t i = 0; i < n_in; ++i) {
                out[(size_t) i] = ggml_fp16_to_fp32(v[(size_t) i]);
            }
            return;
        }
    }
...
void apply_base_block_f32(
    ...
    if (d1) {
        for (int64_t i = 0; i < L; ++i) {
            y_out[(size_t) i] = x_in[(size_t) i] * d1[(size_t) i];
        }
    } else {
